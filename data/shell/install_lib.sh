...
Install_Pcre(){
	...
	if [ "$Cur_Pcre_Ver" == "" ];then
		wget -O pcre-${pcreVersion}.tar.gz ${download_Url}/install/src/pcre-${pcreVersion}.tar.gz -T 5
...
Install_Freetype()
{
	if [ ! -f "/usr/bin/freetype-config" ] && [ ! -f "/usr/local/freetype/bin/freetype-config" ]; then
		cd ${run_path}
		wget -O freetype-${freetypeVersion}.tar.gz ${download_Url}/install/src/freetype-${freetypeVersion}.tar.gz -T 5
...
Install_Libiconv()
{
	...
	if [ ! -f "libiconv-1.14.tar.gz" ];then
		wget -O libiconv-1.14.tar.gz ${download_Url}/install/src/libiconv-1.14.tar.gz -T 5
	fi
	mkdir /patch
	wget -O /patch/libiconv-glibc-2.16.patch ${download_Url}/install/src/patch/libiconv-glibc-2.16.patch -T 5
...
Install_Libmcrypt()
{
	...
	if [ ! -f "libmcrypt-2.5.8.tar.gz" ];then
		wget -O libmcrypt-2.5.8.tar.gz ${download_Url}/install/src/libmcrypt-2.5.8.tar.gz -T 5
	fi
...
Install_Mcrypt()
{
	...
	if [ ! -f "mcrypt-2.6.8.tar.gz" ];then
		wget -O mcrypt-2.6.8.tar.gz ${download_Url}/install/src/mcrypt-2.6.8.tar.gz -T 5
	fi
...
Install_Mhash()
{
	...
	if [ ! -f "mhash-0.9.9.9.tar.gz" ];then
		wget -O mhash-0.9.9.9.tar.gz ${download_Url}/install/src/mhash-0.9.9.9.tar.gz -T 5
	fi
