...
{	
	if [ ! -f "/usr/local/memcached/bin/memcached" ];then
		...
		cd $srcPath
		wget $download_Url/extensions/php/install/lib/memcached-${memcachedVer}.tar.gz -T 5
		...
		wget -O /etc/init.d/memcached $download_Url/extensions/php/install/lib/init.d.memcached -T 5
	...
	if [ ! -f "/usr/local/libmemcached/lib/libmemcached.so" ];then
		cd $srcPath
		wget $download_Url/extensions/php/install/lib/libmemcached-1.0.18.tar.gz -T 5
	...
	if [ ! -f "${extFile}" ];then
		...
		if [ "${version}" -ge "70" ];then
			wget $download_Url/extensions/php/install/lib/memcached-${memcachedPhpVer}.tgz -T 5
			...
			wget $download_Url/extensions/php/install/lib/memcached-2.2.0.tgz -T 5
...
Update_memcached(){
	cd $srcPath
	wget $download_Url/extensions/php/install/lib/memcached-${memcachedVer}.tar.gz -T 20
	tar -xzf memcached-${memcachedVer}.tar.gz
	cd memcached-${memcachedVer}
	./configure --prefix=/usr/local/memcached
	make
