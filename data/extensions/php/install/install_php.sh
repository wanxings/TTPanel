...
Service_Add(){
	...
	if [ "$?" == "127" ];then
		wget -O /usr/lib/systemd/system/php-fpm-${php_version}.service ${download_Url}/service/0.0.1/php/systemd/php-fpm-${php_version}.service
...
Install_Openssl_1_0_2()
{
	if [ ! -f "/usr/local/openssl/bin/openssl" ];then
		cd ${run_path}
		wget ${download_Url}/install/src/openssl-${opensslVersion}.tar.gz
...
Install_Openssl_1_1_1(){
	...
	if [ ! -f "/usr/local/openssl111/bin/openssl" ] && [ -z "${openssl111Check}" ];then
		cd ${run_path}
		wget ${download_Url}/install/src/openssl-${openssl111Version}.tar.gz -T 20
...
Install_Curl()
{
	...
	if [ ! -f "/usr/local/curl/bin/curl" ] || [ "${CURL_OPENSSL_LIB_VERSION}" != "${OPENSSL_LIB_VERSION}" ];then
		wget ${download_Url}/install/src/curl-${curlVersion}.tar.gz
...
Install_Curl2(){
	...
	if [[ "${LibCurlVer}" -le "60" ]]; then
		if [ ! -f "/usr/local/curl2/bin/curl" ];then
			wget ${download_Url}/install/src/curl-${curlVer}.tar.gz
...
Install_Icu4c(){
	...
	if [ ! -f "/usr/bin/icu-config" ] || [ "${icu4cVer:0:2}" -gt "60" ];then
		wget -O icu4c-60_3-src.tgz ${download_Url}/install/src/icu4c-60_3-src.tgz
...
Install_Libzip(){
	...
	if [ "${autoconfVer}" -lt "69" ]; then
		wget ${download_Url}/install/src/autoconf-2.69.tar.gz
...
Install_Onig(){
	...
	if [ -z "${onigCheck}" ];then
		cd ${run_path}
		onigVer="6.9.6"
		wget -O onig-${onigVer}.tar.gz ${download_Url}/install/src/onig-${onigVer}.tar.gz
...
Install_Libsodium(){
	...
	if [ ! -f "/usr/local/libsodium/lib/libsodium.so" ];then
		cd ${run_path}
		libsodiumVer="1.0.18"
		wget ${download_Url}/install/src/libsodium-${libsodiumVer}-stable.tar.gz
...
Download_Src(){
	...
	wget -O src.tar.gz ${download_Url}/service/0.0.1/php/src/php-${phpVersion}.tar.gz
	tar -xvf src.tar.gz
	...
	if [ "${php_version}" == "53" ];then
		...
		mkdir -p /patch
		cd src
		wget -O /patch/php-5.3-multipart-form-data.patch ${download_Url}/service/0.0.1/php/src/patch/php-5.3-multipart-form-data.patch -T20
...
Install_Zend(){
	...
	if [ "${php_version}" -lt "70" ];then
		...
		wget -O php-ZendGuardLoader.tar.gz ${download_Url}/service/0.0.1/php/src/php-ZendGuardLoader.tar.gz
...
Download_Conf(){
	if [ ! -f "/www/server/nginx/conf/enable-php-${php_version}.conf" ];then
		wget -O /www/server/nginx/conf/enable-php-${php_version}.conf ${download_Url}/service/0.0.1/php/conf/enable-php-${php_version}.conf
	fi
}
